...
for store_id, store_df in tqdm( df_master.groupby("store_id", sort=False, observed=False), total=df_master['store_id'].nunique()):
    ...
    cat_series_dict[store_id] = {}

    for cat_id, cat_df in store_df.groupby("cat_id", sort=False, observed=False):

        product_series_list = []
        
        for product_id, product_df in cat_df.groupby("item_id", sort=False, observed=False):
            if product_df.empty:
                continue
            ...
            # --- Static Covariates ---
            static_covs = product_df[["item_id", "dept_id", "release", "price_max", "price_min","price_std"]].iloc[0].to_dict()
        ...
        cat_series_dict[store_id][cat_id] = product_series_list
...
file_path = "/path/to/pklFiles/cat_series_dict.pkl"
# Save the dictionary to a .pkl file
with open(file_path, "wb") as file:
    pickle.dump(cat_series_dict, file)
