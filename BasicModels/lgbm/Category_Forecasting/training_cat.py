...
for store_id, cat_dict in cat_series_dict.items():
    ...
    val_series_cat[store_id]= {}
    #forecast_series_cat[store_id]= {}
    #test_series_cat[store_id]= {}
    future_covs_cat[store_id]= {}
    #series_cat[store_id]= {}
    cat_static_scalers[store_id] = {}
    ...
    for cat_id, ts_list in cat_dict.items():
        ...
        val_series_cat[store_id][cat_id] = val_series
        #forecast_series_cat[store_id][cat_id] = forecast_series
        ...
        val_series_cat[store_id][cat_id] = static_scaler.transform(val_series_cat[store_id][cat_id])
        #forecast_series_cat[store_id][cat_id] = static_scaler.transform(forecast_series_cat[store_id][cat_id])
...
# %%
def train_model(train_series,validation_series,covariates_series):
...
for store_id, cat_dict in cat_series_dict.items():
    for cat_id in cat_dict.keys():
        model = train_model(train_series=train_series_cat[store_id][cat_id],
                            validation_series=val_series_cat[store_id][cat_id],
                            covariates_series=future_covs_cat[store_id][cat_id])
        model.save(f"/path/to/BasicModels/lgbm/Cat_models/{store_id}_{cat_id}")
